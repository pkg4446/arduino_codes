...
String firmwareVersion = "0.0.2"; // 버전만 수정됨 표시
...
////--------------------- PID -------------------------////
#define MAX_BEE_TEMP    35    // 벌통 내부 한계 온도
#define MAX_HEAT        50    // 히터 한계 온도
...
#define PIN_DS_EXT      4     // 히터 온도 센서 (temp_heat)
#define PIN_DS_SPACE    5     // 공기 온도 센서 (temp_air)
#define PIN_CONFIG      2 
////--------------------- Pin out ---------------------////
/*
#define SDA_PIN 21
...
bool heat_use   = false;
////--------------------- PID -------------------------////
uint8_t temp_goal = 3;  // 기본값 안전 설정
float   temp_heater;    // PID가 계산한 히터 목표 온도
//초기 PID값: 수동 튜닝된 보수적 값 권장 (낮/밤 변화 대응)
float Kp = 10.0, Ki = 0.5, Kd = 20.0; 
float lastInput;
float ITerm = 0;
// 자동 튜닝용 변수
bool  isTuning = true;
...
// [개선] DS18B20 비동기 함수들
void requestDS18B20(OneWireNg *ow) {
  if (!ow) return;
...
float readDS18B20_Collect(OneWireNg *ow) {
    if (!ow) return NAN; 
    ...
    if (OneWireNg::crc8(data, 8) != data[8]) return NAN; 
...
// [개선] 안전한 PID 제약 함수
float constraint_f(float val, float min_val, float max_val) {
...
void read_sensors(){
  ...
  if (currentMillis - lastSensorRequest >= SECONDE) { 
        if (!isConversionStarted) {
            // 변환 시작 요청
            requestDS18B20(owExt);
            requestDS18B20(owSpace);
            ...
            lastSensorRequest = currentMillis; // 시작 시간 기록
            isConversionStarted = true;
        }
    }
    // 변환 시간(750ms)이 지났는지 확인
    if (isConversionStarted && (currentMillis - lastSensorRequest >= DS18B20_DELAY)) {
        // 데이터 수집
        float th = readDS18B20_Collect(owExt);
        float ta = readDS18B20_Collect(owSpace);
        
        if (!isnan(th)) temp_heat = th;
        if (!isnan(ta)) temp_air = ta;
        
        isConversionStarted = false; // 플래그 초기화 (다음 1초 주기 대기)
    }
}
////--------------------- PID -------------------------////
float constraint(float val, float min_val, float max_val) {
  if (val < min_val) return min_val;
  if (val > max_val) return max_val;
  return val;
}
...
void run_pid() {
  // 1. 안전 장치: 필수 센서 에러 시 히터 차단
  if (isnan(temperature) || isnan(temp_air) || isnan(temp_heat)) {
    digitalWrite(PIN_SSR_HEATER, false);
    return;
  }
  // 2. 안전 장치: 벌통 과열 방지
  if (temperature >= MAX_BEE_TEMP) {
     digitalWrite(PIN_SSR_HEATER, false);
     return;
  }
  ...
  // 출력이 포화 상태가 아니고, 오차의 방향이 포화 방향과 같지 않을 때만 적분
  bool isSaturated = (temp_heater >= MAX_HEAT && error > 0) || (temp_heater <= 0 && error < 0);  
  if (!isSaturated) {
      ITerm += (Ki * error * timeChange);
  }

  // Anti-windup (출력 제한)
  ITerm = constraint(ITerm, 0, MAX_HEAT);

  float dInput = (temp_air - lastInput) / timeChange;

  temp_heater = Kp * error + ITerm - Kd * dInput;
  temp_heater = constraint(temp_heater, 0, MAX_HEAT);
...
void serial_command_help() {
  ...
  Serial.println("tune     * PID Auto tune.");
